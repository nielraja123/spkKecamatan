...
class Rangking extends MY_Controller
{
    ...
    {
        $kecamatan = $this->MKecamatan->getAll();
         ...
         */
        $table1 = $this->initialTableSAW($kecamatan);
    ...
    private function initialTableSAW($kecamatan)
    {
        $nilai = $this->MNilai->getNilaiUniveristas();

        $dataInput = array();
        $no = 0;
        foreach ($kecamatan as $item => $itemKecamatan) {
            foreach ($nilai as $index => $itemNilai) {
                if ($itemKecamatan->kdKecamatan == $itemNilai->kdKecamatan) {
                    $dataInput[$no]['kecamatan'] = $itemKecamatan->kecamatan;
